...
use Rasuvaeff\PropertyTesting\Internal\Clock;
use Rasuvaeff\PropertyTesting\Internal\DrawContext;
use Rasuvaeff\PropertyTesting\Internal\MonotonicClock;
    ...
    public function run(
        ...
        if ($corpus instanceof Corpus && $property->replayRegressions) {
            foreach ($corpus->recall($property->id, $property->parameterNames) as $entry) {
                if ($entry->isValues()) {
                    $this->emit($listeners, new CorpusReplayed($property->id, true, $entry->arguments, $entry->seed));
                    ...
                    $this->emit($listeners, new CorpusReplayed($property->id, false, [], $entry->seed));
