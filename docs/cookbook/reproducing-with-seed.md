...
description: "Three ways to replay a falsified property's reported seed exactly, and how PROPERTY_VERBOSE fills in what happened during that run."
---
...
than a best-effort hint. See [`Random`](/api/classes/Random).
...
that, prefer [explicit examples](/explicit-examples) or the
[regression corpus](/regression-corpus) — both are designed to persist,
...
your only defense — see [Regression corpus](/regression-corpus) for the
mechanism (`PROPERTY_DB`) that already accounts for this via
`SEQUENCE_EPOCH`, and [explicit examples](/explicit-examples) for pinning
