...
- Dozens of built-in generators (`Gen::int()`, `Gen::string()`,
  ...
  [Generators](/en/generators/index).
- **Integrated shrinking**: every generator already knows how to minimize its
  own values — see [Shrinking](/en/shrinking).
- A path beyond single inputs, into random *sequences* of operations against
  a stateful system — see [State machine testing](/en/state-machine/concepts).

## Next

[Getting started](/en/intro/getting-started) installs the package and walks
through writing the first property. [Concepts](/en/intro/concepts) is a
