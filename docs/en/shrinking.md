...
Since 2.0, shrinking is **integrated into generation**, not a separate step.
Every `ArbitraryInterface::generate(Random)` call returns a
[`Shrinkable`](https://github.com/rasuvaeff/property-testing/blob/master/src/Shrinkable.php):
the drawn value, plus a lazy tree of smaller candidates attached right there
at generation time. There is no standalone `shrink(mixed $value)` method that
has to reverse-engineer a smaller value from a bare scalar — the tree already
exists, built by the same code that produced the value.
...
[Bounding shrink work](/en/controlling-runs/bounding-shrink).
...
[custom `ArbitraryInterface`](/en/generators/custom-arbitrary) you write),
make that guarantee:
...
2. **No candidate equals its parent.** Each builder decreases some concrete
   ...
   into an ordered domain — strictly on every step. The interceptor adds a
...
[Custom arbitrary](/en/generators/custom-arbitrary).
