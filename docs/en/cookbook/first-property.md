...
- A sibling method named `<methodName>Generators` (or named explicitly via
  ...
  parameter, keyed by parameter name. See [Generators](/en/generators/index)
...
[Assume vs filter](/en/controlling-runs/assume-vs-filter)) don't count toward
that total.
...
- **`seed`** — feed it back via `#[Property(seed: 918273645)]` to replay this
  exact run; see [Reproducing with a seed](/en/cookbook/reproducing-with-seed).
...
- **`Shrunk`** — the smallest input the shrinker could find that still fails,
  ...
  [Shrinking](/en/shrinking) for how the descent works and why it terminates.
...
- [Concepts](/en/intro/concepts) for the vocabulary (arbitrary, shrinking,
  counterexample) this page used without defining.
- [Generators overview](/en/generators/index) for the full `Gen::*` catalog.
- [Reproducing with a seed](/en/cookbook/reproducing-with-seed) once you've
