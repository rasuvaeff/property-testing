...
description: "Реализация ArbitraryInterface напрямую для доменного генератора, с shrink-деревом, которое завершается и никогда не повторяет значение родителя."
---
