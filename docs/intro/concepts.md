...
See [Getting started](/intro/getting-started).
...
See [Generators](/generators/index).
...
**Discard**
&nbsp;&nbsp;A run abandoned via [`Assume::that(false)`](/controlling-runs/assume-vs-filter)
...
See [Shrinking](/shrinking) for how the tree itself is built and why it's
...
[`PropertyViolationException`](/api/exceptions).
...
unnoticed. See [Regression corpus](/regression-corpus).
