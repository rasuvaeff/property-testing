...
- Freeze this package. `2.8.1` was the last functional release; the package is
  marked `abandoned` in favour of `rasuvaeff/property-testing-testo`, and only
  security fixes will follow. Development continues in
  `rasuvaeff/property-testing-core` (engine),
  `rasuvaeff/property-testing-testo` (Testo, drop-in: same FQCNs, same
  conventions, same corpus format) and `rasuvaeff/property-testing-phpunit`.
