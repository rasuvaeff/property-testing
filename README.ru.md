...
Свойство, отбросившее **каждый** прогон, не сделало ни одной успешной проверки,
поэтому «passed» был бы вырожденным — бегун проваливает его с `GaveUpException`
вместо тихого зелёного. Стройте валидные входы (`Gen::flatMap()` / `Gen::draw()`),
а не отбрасывайте широко.
...
```
Отброшенные прогоны (`Assume::that()`) исключаются из знаменателя. Свойство,
все прогоны которого отброшены, не сделало ни одной успешной проверки и
проваливается сразу (см. `GaveUpException` в разделе `Assume::that()`). @@ЛИНИЯ@@
### Выборка генератора
