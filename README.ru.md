...
Строка `Changed:` — diff исходного и минимизированного контрпримера: аргументы,
которые shrinker не тронул (здесь `cap`), опущены, так что входы, реально
влияющие на падение, видны сразу. `trial(s)` — сколько кандидатов shrinker
прогнал всего (принятых и отвергнутых); `shrink step(s)` — только принятые.
...
`CounterExample::toArray()` и `toJson()` возвращают нормализованное представление
для reporters и CI artifacts. `toExamplesCode('delayExamples')` создаёт готовый
метод examples для shrunk scalar/array/enum аргументов; для объектов, которые
нельзя надёжно восстановить как PHP-код, метод бросает `LogicException`.
...
| Фабрика | Производит | Сжимается |
 ...
 | `Gen::dictOf($key, $value, $min, $max)` | `DictionaryArbitrary`, карты с различными ключами из `$key` (int/string) и значениями из `$value`, размер 0..100 по умолчанию | в сторону `[]`, затем по размеру, затем по каждому значению (ключи фиксированные) |
...
| `Gen::floatSpecial()` | OneOfArbitrary для NAN, ±INF, -0.0 и ребер представления с плавающей запятой | к ранее перечисленным специальным предложениям |
 ...
 | `Gen::filter($inner, $predicate)` | `FilteredArbitrary`, значения `$inner`, удовлетворяющие `$predicate` (бросает `GenerationExhausted` после 100 отклонённых попыток — никогда не выдаёт значение вне домена) | внутреннее дерево, обрезка кандидатов, не выполнивших предикат |
...
Генераторы с размером гарантируют **минимум**: `uniqueArrayOf`/`dictOf` (различные
элементы/ключи) и `commands` (применимые шаги) могут выдать меньше *запрошенного*
размера, когда пространство значений исчерпано, но никогда меньше `$min` —
недостижимый минимум бросает `GenerationExhausted`, а не отдаёт свойству
...
### `Assume::that()`
Отбрасывает текущую попытку, если предварительное условие не выполняется. `runs`
задаёт число успешных проверок, поэтому отброшенные попытки заменяются новыми.
При доле discard выше 90% runner предупреждает о плохом распределении генераторов.
```php
Assume::that($cap >= $baseSeconds);
```

Число отброшенных попыток ограничивает `maxDiscards` (по умолчанию `runs * 10`).
После превышения budget property завершается `GaveUpException` с числами
required/successful/discarded/attempts. Стройте валидные входы через
`Gen::flatMap()` / `Gen::draw()`, когда зависимость можно выразить напрямую.
...
### Deadline и бюджет времени

Патологические входы (катастрофический regex, глубокая рекурсия, неограниченный
backoff) проявляются как время, а не как упавшие assertions. Два опциональных
wall-clock ограничения превращают их в явные падения:

```php
#[Property(runs: 200, timeoutMs: 100, budgetMs: 5_000)]
```

- `timeoutMs` — deadline **одного прогона** (случайного или explicit example).
  Тело дольше лимита проваливает property с `DeadlineExceededException`,
  называя виновный вход и измеренное время. Вход сообщается как есть, без
  shrink (принятие shrink-кандидата пришлось бы мерить по времени заново, а
  таймерный шум делает такой спуск недетерминированным). Время меряется после
  возврата тела — зависшее тело в синхронном PHP прервать нельзя.
- `budgetMs` — бюджет **всей случайной фазы**. Если он исчерпан до завершения
  `runs` успешных проверок, property падает с `TimeBudgetExceededException`
  с числами completed/required — медленное property не может молча проверить
  меньше, чем заявляет.

Оба по умолчанию `null` (выключены). Упавший assertion в медленном прогоне
приоритетнее deadline — фальсифицированный контрпример важнее.
...
Две переменные среды позволяют настроить запуск без изменения атрибутов — полезно в
 ...
 | `PROPERTY_VERBOSE` | Любое значение, кроме `''`/`0`, регистрирует сгенерированные аргументы каждого запуска и, при падении, каждый принятый шаг сжатия (`shrink step 3: x=63 -> 51`) — видно и что воспроизведённый seed передаёт свойству, и как спускается shrinker. |
...
```
Отброшенные попытки (`Assume::that()`) исключаются из знаменателя и заменяются,
пока не выполнены все успешные runs. Превышение `maxDiscards` проваливает
property через `GaveUpException`.
### Выборка генератора
